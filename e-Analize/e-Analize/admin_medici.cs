...
namespace e_Analize
{
    public partial class admin_medici : Form
    {
        SqlConnection con;
        SqlCommand cmd;
        SqlDataAdapter da;
        ...
        void get_total()
        {
            cmd.CommandText = "SELECT COUNT(Id) FROM Medici WHERE Nume LIKE '%'+@Nume+'%' OR Prenume LIKE '%'+@Prenume+'%'";
        ...
        void go_to_position()
        {
            cmd.CommandText = "SELECT  * FROM Medici WHERE Nume LIKE '%'+@Nume+'%' OR Prenume LIKE '%'+@Prenume+'%' ORDER BY Id ASC OFFSET " + position + " ROWS FETCH NEXT 1 ROWS ONLY";
            cmd.Parameters.Clear();
            cmd.Parameters.Add("@Nume", SqlDbType.NVarChar);
            cmd.Parameters.Add("@Prenume", SqlDbType.NVarChar);
            cmd.Parameters["@Nume"].Value = search_text;
            ...
            visual_datetimepicker_dataangajarii.Value = (DateTime)(ds.Tables[0].Rows[0].ItemArray[5]);
            if ((bool)(ds.Tables[0].Rows[0].ItemArray[6])) { visual_radiobutton_masculin.Select(); } else { visual_radiobutton_feminin.Select(); }
            visual_textbox_telefon.Text = ds.Tables[0].Rows[0].ItemArray[7].ToString();
            visual_textbox_email.Text = ds.Tables[0].Rows[0].ItemArray[8].ToString();
            visual_textbox_adresa.Text = ds.Tables[0].Rows[0].ItemArray[9].ToString();
            visual_picturebox.ImageLocation = Application.StartupPath + "/e-Analize_data/Photo/Medici/" + ds.Tables[0].Rows[0].ItemArray[10].ToString();
        ...
        void change_state(string s)
        {
            ...
            if (s == "add")
            {
                ...
                visual_datetimepicker_dataangajarii.Value = DateTime.Now;
        ...
        private void visual_button_add_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                cmd.CommandText = "INSERT INTO dbo.Medici(Nume,Prenume,CNP,Data_Nasterii,Data_Angajarii,Sex,Telefon,Email,Adresa,Image) Values(@Nume,@Prenume,@CNP,@Data_Nasterii,@Data_Angajarii,@Sex,@Telefon,@Email,@Adresa,@Image)";
                ...
                cmd.Parameters["@Data_Angajarii"].Value = visual_datetimepicker_dataangajarii.Value;
                ...
                if (imagepath != "") File.Copy(imagepath, Application.StartupPath + "/e-Analize_data/Photo/Medici/" + timestamp.ToString()+".jpg");
